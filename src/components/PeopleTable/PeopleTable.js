...
function PeopleTable() {
    ...
    const displayDataValues = (value, dataIndex) => {
        if(dataIndex === 10) {
            // For Species
            if(value?.length === 0) {
                return <td key={dataIndex}><i class="fa fa-circle"></i>{" Human"}</td>
            }
            if(value?.[0] === 'Droid') {
                return <td key={dataIndex}><i class="fa fa-brands fa-android"></i>{" " + value}</td>
            }
            return <td key={dataIndex}><i class="fa fa-question-circle" aria-hidden="true"></i></td>
        }
    ...
    return (
        <>
            ...
            {isLoading && <span class="fa fa-spinner fa-pulse loaderIcon"></span>}
